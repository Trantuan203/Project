...
const Appearance = () => {
  ...
  const handleThemeModeChange = (nextThemeMode) => {
    ...
    setFeedback({
      message: 'Display Theme da duoc ap dung ngay.',
      type: 'success',
    });
  };
  ...
  const handleCustomWallpaperChange = (event) => {
    ...
    if (!file.type.startsWith('image/')) {
      setFeedback({
        message: 'Chi ho tro file anh cho custom wallpaper.',
        type: 'error',
      });
      return;
    }
    ...
    if (file.size > MAX_CUSTOM_WALLPAPER_SIZE_BYTES) {
      setFeedback({
        message: 'Anh custom vuot qua gioi han 1.5MB de luu local tren trinh duyet.',
        type: 'error',
      });
      return;
    }
    ...
    reader.onload = () => {
      if (typeof reader.result !== 'string') {
        setFeedback({
          message: 'Khong doc duoc anh da chon. Thu file khac.',
          type: 'error',
        });
        return;
      }
    ...
    reader.onerror = () => {
      setFeedback({
        message: 'Khong tai duoc anh custom. Thu lai file khac.',
        type: 'error',
      });
    };
  ...
  const handleApply = () => {
    ...
    setFeedback({
      message: 'Text size va wallpaper da duoc ap dung tren may nay.',
      type: 'success',
    });
  };
    ...
    <section className="space-y-12 rounded-[32px] bg-surface-container-lowest p-6 shadow-sm md:p-8">
      ...
      <section>
        ...
        <div className="grid items-start gap-8 xl:grid-cols-[minmax(0,360px)_minmax(0,1fr)]">
          ...
          <div
            className="space-y-4 rounded-[28px] border border-outline-variant/10 p-6"
      ...
      <section>
        ...
        <p className="mt-4 text-xs leading-5 text-on-surface-variant">
          Preset va custom wallpaper hien duoc luu local tren may nay. Neu ban muon dong bo
          theo tai khoan tren nhieu thiet bi thi can backend/API.
        </p>
      </section>
